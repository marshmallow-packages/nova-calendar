...
By default, the calendar is exposed to your end users under the `marshmallow/nova-calendar` URI.
...
If you are going to return a long list of events here, or do a request to an external service, you can use the `startOfCalendar()` and `endOfCalendar()` methods inherited from `Marshmallow\NovaCalendar\DataProvider\AbstractDataProvider` to limit the scope of your event generation to the date range that is currently being requested by the frontend.
