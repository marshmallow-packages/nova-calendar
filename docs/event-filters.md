...
Event filters do _not_ combine; the user either applies 1 event filter, or none.
...
```php
use Marshmallow\NovaCalendar\EventFilter\NovaResourceFilter;
...
```php
use Marshmallow\NovaCalendar\EventFilter\ExcludeNovaResourceFilter;
...
```php
use Marshmallow\NovaCalendar\EventFilter\EloquentCallbackFilter;
...
```php
use Marshmallow\NovaCalendar\EventFilter\CallbackFilter;
...
Since this filter is applied to any event on your calendar, the event is _not_ strictly guaranteed to contain a Nova resource and an Eloquent model before the callback method gets called, so you need to do checks before using the `model()` and `resource()` methods. If you want the filter to show only calendar events that have underlying Eloquent models anyway, use an [`EloquentCallbackFilter`](#eloquentcallbackfilter) instead.
...
Implement a subclass of `Marshmallow\NovaCalendar\EventFilter\AbstractEventFilter`. The only method that you have to implement is `showEvent(Event $event): bool`. Implement it however you see fit and you can add an instance of the custom filter class to the `filters()` method in your CalendarDataProvider.
...
```php
use Marshmallow\NovaCalendar\EventFilter\AbstractEventFilter;
