...
1. Publish the config file by running the following command:
    ...
    - the _calendar key_, which is the string array key in the config file (`my-calendar`)
    - _dataProvider_, which is the class that will generate the calendar event data (`App\Providers\CalendarDataProvider`)
...
1. Create the data provider class:
    ...
    If you choose to make the data provider yourself, be sure to make it a subclass of `Marshmallow\NovaCalendar\DataProvider\AbstractDataProvider`.
    ...
    use Marshmallow\NovaCalendar\DataProvider\AbstractDataProvider;
...
1. In the data provider created under step 3, implement the `novaResources()` method to specify which Nova resources are to be included and which of their model's `DateTime` attributes define when the event starts and, optionally, when it ends.
    ...
    use Marshmallow\NovaCalendar\DataProvider\MonthCalendar;
    use App\Nova\User;
